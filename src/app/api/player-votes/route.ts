...
export async function POST(request: NextRequest) {
  try {
    ...
    const { playerSlug, type } = body; // type: 'star' | 'bagre'

    if (!playerSlug || (type !== 'star' && type !== 'bagre')) {
    ...
    if (supabase) {
      try {
        const column = type === 'star' ? 'star_count' : 'bagre_count';
        ...
        if (!upsertErr) {
          return NextResponse.json({
            slug: cleanSlug,
            type,
            count: type === 'star' ? newStar : newBagre,
          });
        }
      } catch (e) {
        console.error('Supabase vote update error, falling back to CounterAPI:', e);
      }
    }
    ...
    return NextResponse.json({ slug: cleanSlug, type, count: 1 });
  } catch (e) {
