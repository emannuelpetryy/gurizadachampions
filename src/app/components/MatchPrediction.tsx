...
export default function MatchPrediction({ 
  ...
  const defaultA = 10 + (matchId % 5) * 3;
  const defaultB = 8 + (matchId % 3) * 4;
  const [votes, setVotes] = useState({ a: defaultA, b: defaultB });
  ...
  return (
    <div style={{ width: '100%', background: 'rgba(13, 20, 36, 0.8)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(0,240,255,0.15)', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{total} Votos</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.8rem' }}>
        ...
        >
          <span>{teamAName}</span>
          <span style={{ color: 'var(--cyan)', fontFamily: 'var(--font-rajdhani)', fontSize: '1.1rem', fontWeight: 'bold' }}>{pctA}%</span>
        </button>
        ...
        >
          <span>{teamBName}</span>
          <span style={{ color: 'var(--accent-red)', fontFamily: 'var(--font-rajdhani)', fontSize: '1.1rem', fontWeight: 'bold' }}>{pctB}%</span>
        </button>
      </div>
      ...
      <div style={{ width: '100%', height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden', display: 'flex' }}>
        <div style={{ width: `${pctA}%`, height: '100%', background: 'var(--cyan)', transition: 'width 0.3s' }}></div>
        <div style={{ width: `${pctB}%`, height: '100%', background: 'var(--accent-red)', transition: 'width 0.3s' }}></div>
      </div>
    </div>
  );
}
